...
func Load() Config {
	cfg := Config{
		WorkerName:        "cluster-addon-worker",
		ListenAddr:        ":8095",
		PollInterval:      10 * time.Second,
		HeartbeatInterval: 20 * time.Second,
		BackendBaseURL:    "http://127.0.0.1:18080",
	}
	if path := resolveConfigFile("CLUSTER_ADDON_WORKER_CONFIG_FILE", "config/cluster-addon-worker.yaml", "cluster-addon-worker.yaml", "config.yaml"); path != "" {
		if err := applyConfigFile(path, &cfg); err != nil {
			panic(fmt.Errorf("parse config file %s: %w", path, err))
		}
		cfg.ConfigFile = path
	}
	if value := os.Getenv("CLUSTER_ADDON_WORKER_NAME"); value != "" {
		cfg.WorkerName = value
	}
	if value := os.Getenv("CLUSTER_ADDON_WORKER_LISTEN_ADDR"); value != "" {
		cfg.ListenAddr = value
	}
	if value := os.Getenv("CLUSTER_ADDON_WORKER_POLL_INTERVAL"); value != "" {
		if parsed, err := time.ParseDuration(value); err == nil {
			cfg.PollInterval = parsed
		}
	}
	if value := os.Getenv("CLUSTER_ADDON_WORKER_HEARTBEAT_INTERVAL"); value != "" {
		if parsed, err := time.ParseDuration(value); err == nil {
