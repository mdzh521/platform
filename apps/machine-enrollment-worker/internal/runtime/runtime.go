...
import (
	...
	"machine-enrollment-worker/internal/client/controlplane"
	"machine-enrollment-worker/internal/config"
	"machine-enrollment-worker/internal/worker"
)
...
func (r *Runtime) Run(ctx context.Context) error {
	go func() {
		log.Printf("machine-enrollment-worker listening on %s", r.server.Addr)
		if err := r.server.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			log.Printf("machine-enrollment-worker http stopped: %v", err)
		}
	}()
