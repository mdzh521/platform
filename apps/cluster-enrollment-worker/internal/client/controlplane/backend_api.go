...
func NewBackendAPI(baseURL, token string) *BackendAPI {
	return &BackendAPI{baseURL: strings.TrimRight(baseURL, "/"), token: token, client: &http.Client{Timeout: 30 * time.Second}}
}

func (b *BackendAPI) Claim(ctx context.Context, workerName string) (*domain.Claim, error) {
	payload, err := b.request(ctx, http.MethodPost, "/api/v1/cloud/internal/cluster-enrollment/claim", map[string]any{"worker_name": workerName})
...
func (b *BackendAPI) Report(ctx context.Context, resourceID uint, result domain.Result) error {
	_, err := b.request(ctx, http.MethodPost, fmt.Sprintf("/api/v1/cloud/internal/cluster-enrollment/%d/result", resourceID), map[string]any{
