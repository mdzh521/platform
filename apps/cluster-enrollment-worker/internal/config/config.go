...
func Load() Config {
	cfg := Config{
		WorkerName:     "cluster-enrollment-worker",
		ListenAddr:     ":8094",
		PollInterval:   10 * time.Second,
		BackendBaseURL: "http://127.0.0.1:18080",
	}
	if path := resolveConfigFile("CLUSTER_ENROLLMENT_WORKER_CONFIG_FILE", "config/cluster-enrollment-worker.yaml", "cluster-enrollment-worker.yaml", "config.yaml"); path != "" {
		if err := applyConfigFile(path, &cfg); err != nil {
			panic(fmt.Errorf("parse config file %s: %w", path, err))
		}
		cfg.ConfigFile = path
	}
	if value := os.Getenv("CLUSTER_ENROLLMENT_WORKER_NAME"); value != "" {
		cfg.WorkerName = value
	}
	if value := os.Getenv("CLUSTER_ENROLLMENT_WORKER_LISTEN_ADDR"); value != "" {
		cfg.ListenAddr = value
	}
	if value := os.Getenv("CLUSTER_ENROLLMENT_WORKER_POLL_INTERVAL"); value != "" {
