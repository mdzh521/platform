...
function renderSummaryCards(view) {
  ...
  elements.cloudSummaryCards.innerHTML = `
    <article class="cloud-summary-card">
      <span class="muted-label">交付上下文</span>
      <strong>${view.activeAccounts}</strong>
      <p>${view.activeAccounts > 0 ? "已有可用账号，可以继续收口 Foundation Network。" : "还没有可用云账号。"}</p>
    </article>
    <article class="cloud-summary-card">
      <span class="muted-label">Foundation Network</span>
      <strong>${view.networkPlans}</strong>
      <p>${view.succeededFoundationJobs > 0 ? `${view.succeededFoundationJobs} 条基础网络交付已成功。` : "基础网络可以先创建，成功执行后才算真正落地。"}</p>
    </article>
    <article class="cloud-summary-card">
...
function renderFoundationStatus(view) {
  ...
  elements.cloudFoundationStatus.innerHTML = `
    <article class="cloud-status-card ${view.activeAccounts > 0 ? "ready" : "blocked"}">
      <span class="muted-label">Foundation Accounts</span>
      <strong>${view.activeAccounts > 0 ? "已就绪" : "缺少账号"}</strong>
      <p>${view.activeAccounts > 0 ? `当前有 ${view.activeAccounts} 组可用账号。` : "至少需要一组可用账号，foundation 才能真正落地。"}</p>
    </article>
    <article class="cloud-status-card ${view.networkPlans > 0 ? "active" : "blocked"}">
      <span class="muted-label">Foundation Network</span>
      <strong>${view.networkPlans > 0 ? "已定义" : "未建立"}</strong>
      <p>${view.networkPlans > 0 ? `当前已保存 ${view.networkPlans} 条 foundation network 记录。` : "先创建 foundation network，收口命名、角色和 provider-specific refs。"}</p>
    </article>
    <article class="cloud-status-card ${view.succeededFoundationJobs > 0 ? "ready" : "waiting"}">
      <span class="muted-label">Foundation Delivery</span>
      <strong>${view.succeededFoundationJobs > 0 ? "已成功落地" : "等待首次落地"}</strong>
      <p>${view.succeededFoundationJobs > 0 ? `${view.succeededFoundationJobs} 条 foundation delivery 已成功完成，可继续复用 outputs。` : "有 foundation network 还不够，至少要有一条成功的 foundation delivery 记录。"}</p>
    </article>
  `;
}
...
function buildCloudOverview() {
  ...
  if (activeAccounts === 0) {
    return {
      ...
      nextActionCopy: "账号建好以后，再去创建 Foundation Network。",
  ...
  if (Number(summary.networks || 0) === 0) {
    return {
      ...
      stageCopy: "账号已经有了，现在直接创建 Foundation Network 就行。",
      blockerTitle: "还没有基础网络",
      blockerCopy: "没有 Foundation Network，就不能创建真正可复用的服务器、Bastion 或 Cluster。",
      latestResultTitle: latestJob ? `${latestJob.name || "最近任务"} · ${statusLabel(latestJob.status || "-")}` : "还没有任务记录",
      latestResultCopy: latestJob ? `${latestJob.blueprint_name || "-"} · ${actionLabel(latestJob.action || "-")}` : "先创建第一条 Foundation Network。",
      nextActionTitle: "点“创建 Foundation Network”",
      nextActionCopy: "先完成一条基础网络记录，再从网络继续创建服务器或其他交付对象。",
      primaryAction: "create-network",
      primaryActionLabel: "创建 Foundation Network",
  ...
  if (!foundationComplete) {
    return {
      ...
      stageCopy: "Foundation Network 虽然已经建了，但还没形成可复用的成功结果。先看网络详情和任务状态，确认底座落地。",
      ...
      latestResultCopy: latestJob ? `${latestJob.blueprint_name || "-"} · ${actionLabel(latestJob.action || "-")}` : "先确认 Foundation 为什么还没成功。",
      nextActionTitle: "先看 Foundation Network 和任务状态",
      nextActionCopy: "Foundation 创建时会自动发起执行；当前先确认它为什么还没成功。",
      primaryAction: "scroll-networks",
      primaryActionLabel: "查看 Foundation Network",
  ...
  if (!bastionSucceeded) {
    return {
      ...
      stageCopy: "现在最短路径是从某条 Foundation Network 继续创建服务器或 Ops Bastion，不要先跳到脱离网络的通用任务。",
      ...
      nextActionCopy: "优先从 Foundation Network 卡片点“创建服务器”或“创建 Ops Bastion”。",
      primaryAction: "scroll-networks",
  ...
  return {
    ...
    nextActionCopy: "先确认结果没问题，再从已有 Foundation Network 扩展服务器、Bastion 或 Cluster。",
    primaryAction: "scroll-networks",
    primaryActionLabel: "从网络继续创建",
    secondaryAction: "scroll-jobs",
    secondaryActionLabel: "查看任务状态",
