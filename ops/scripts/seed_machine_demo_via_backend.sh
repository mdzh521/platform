...
api_get() {
  local path="$1"
  /usr/local/bin/docker compose -f /Users/alex/ops/platform-center/ops/compose/docker-compose.yml exec -T backend /bin/sh -lc "wget -qO- --header='Authorization: Bearer $TOKEN' http://127.0.0.1:8080$path"
}

api_post() {
  local path="$1"
  local payload="$2"
  /usr/local/bin/docker compose -f /Users/alex/ops/platform-center/ops/compose/docker-compose.yml exec -T backend /bin/sh -lc "wget -qO- --post-data='$payload' --header='Authorization: Bearer $TOKEN' --header='Content-Type: application/json' http://127.0.0.1:8080$path"
}

TOKEN=$(/usr/local/bin/docker compose -f /Users/alex/ops/platform-center/ops/compose/docker-compose.yml exec -T backend /bin/sh -lc "wget -qO- --post-data='{\"username\":\"admin\",\"password\":\"admin123456\"}' --header='Content-Type: application/json' http://127.0.0.1:8080/api/v1/auth/login/local" | sed -n 's/.*"token":"\([^"]*\)".*/\1/p')
...
create_group() {
  ...
  existing=$(api_get "/api/v1/machines/groups" | grep -c "\"name\":\"$name\"" || true)
  if [ "$existing" = "0" ]; then
    payload=$(printf '{"name":"%s","code":"%s","default_login_policy":"%s","description":"%s"}' "$name" "$code" "$policy" "$desc")
    api_post "/api/v1/machines/groups" "$payload" >/dev/null
  fi
}
...
create_asset() {
  ...
  local port="$5"
  local account="$6"
  local asset_status="$7"
  local tags="$8"
  local desc="$9"
  local existing
  local payload

  existing=$(api_get "/api/v1/machines/assets" | grep -c "\"name\":\"$name\"" || true)
  if [ "$existing" = "0" ]; then
    payload=$(printf '{"name":"%s","address":"%s","platform":"linux","protocol":"ssh","group_name":"%s","login_policy":"%s","port":%s,"account":"%s","status":"%s","tags":"%s","description":"%s"}' \
      "$name" "$address" "$group" "$policy" "$port" "$account" "$asset_status" "$tags" "$desc")
    api_post "/api/v1/machines/assets" "$payload" >/dev/null
  fi
}
...
echo "GROUPS"
api_get "/api/v1/machines/groups"
echo
echo "ASSETS"
api_get "/api/v1/machines/assets"
